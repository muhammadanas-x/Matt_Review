#include "sobol_bench_common.cuh"

constexpr uint32_t NUM_MATRIX_SLOTS =
    DEPTH * SLOTS_PER_DEPTH;

__device__ __constant__
uint16_t g_matrixSlots[NUM_MATRIX_SLOTS][16];

static uint16_t
    h_matrixSlots[NUM_MATRIX_SLOTS][16];

__device__ __forceinline__
uint16_t row_major_matrix_apply(
    const uint16_t* rows,
    uint16_t sampleIx)
{
    // Text-exact port of Nabla RowMajorSobolMatrix::operator().
    const uint16_t keepMask =
        uint16_t(0b10000100001u);

    uint16_t val =
        uint16_t(
            __popc(
                uint32_t(rows[0] & sampleIx)));

    val |=
        uint16_t(
            __popc(
                uint32_t(rows[5] & sampleIx))
            << 5);

    val |=
        uint16_t(
            __popc(
                uint32_t(rows[10] & sampleIx))
            << 10);

    val |=
        uint16_t(
            __popc(
                uint32_t(rows[15] & sampleIx))
            << 15);

    val &= keepMask;

#pragma unroll
    for (uint16_t i = 1u;
         i < 5u;
         ++i)
    {
        uint16_t tmp =
            uint16_t(
                __popc(
                    uint32_t(rows[i] & sampleIx)));

        tmp |=
            uint16_t(
                __popc(
                    uint32_t(rows[5u + i] & sampleIx))
                << 5);

        tmp |=
            uint16_t(
                __popc(
                    uint32_t(rows[10u + i] & sampleIx))
                << 10);

        val |=
            uint16_t(
                (tmp & keepMask) << i);
    }

    return val;
}


__global__ void sobol_bench_kernel(
    uint32_t* output,
    const uint2_32* seedTexture,
    const QuantizedSequence3* pSampleBuffer, // BDA-equivalent flat pointer, unused for METHOD 1
    PushConstants pc)
{
    const uint32_t invID = blockIdx.x * blockDim.x + threadIdx.x;

    const uint16_t rank = uint16_t(bit_reverse_32(invID) >> 16);

    const uint16_t laneOffset =
        uint16_t(invID & pc.depthStaggerMask) & uint16_t(DEPTH - 1u);

    const float thresholdScale = 65535.0f;
    float threshold = pc.probAtDepth0 * thresholdScale;

    const float thresholdDelta =
        (pc.probAtDepthMax - pc.probAtDepth0) * thresholdScale / float(DEPTH - 1);

    uint16_t seed = uint16_t(0xdeadu);

    for (uint16_t s = 0; s < BENCH_ITERS; s++)
    {
        float perSampleThreshold = threshold;

        for (uint16_t d = 0; d < DEPTH; d++)
        {
            const uint16_t dIdx = uint16_t((d + laneOffset) & uint16_t(DEPTH - 1u));

            uint16_t triples[TRIPLETS][COMPONENTS];

#pragma unroll
            for (uint16_t t = 0; t < TRIPLETS; t++)
            {
#pragma unroll
                for (uint16_t c = 0; c < COMPONENTS; c++)
                {
                    const uint32_t slot = (uint32_t(dIdx) * TRIPLETS + uint32_t(t)) * COMPONENTS + uint32_t(c);
                    triples[t][c] = row_major_matrix_apply(g_matrixSlots[slot], uint16_t(s ^ rank));
                }
            }

            seed ^= triples[0][0];
            seed += triples[0][1];
            seed ^= triples[0][2];

            seed += triples[1][0];
            seed ^= triples[1][1];
            seed += triples[1][2];

            if (triples[0][2] < uint16_t(perSampleThreshold))
                break;

            perSampleThreshold += thresholdDelta;
        }
    }

    output[invID] = uint32_t(seed);
}

static void prepare_method_data()
{
    for (uint32_t slot = 0u;
         slot < NUM_MATRIX_SLOTS;
         ++slot)
    {
        for (uint32_t i = 0u;
             i < 16u;
             ++i)
        {
            h_matrixSlots[slot][i] =
                uint16_t(0xdeadu);
        }
    }

    CUDA_CHECK(
        cudaMemcpyToSymbol(
            g_matrixSlots,
            h_matrixSlots,
            sizeof(h_matrixSlots)));
}

static void launch_method_kernel(
    uint32_t* output,
    const uint2_32* seedTexture,
    const QuantizedSequence3* sampleBuffer,
    PushConstants pc,
    uint32_t blocks,
    uint32_t threads)
{
    sobol_bench_kernel<<<blocks, threads>>>(
        output,
        seedTexture,
        sampleBuffer,
        pc);
}

int main()
{
    return run_sobol_benchmark(
        "1",
        prepare_method_data,
        launch_method_kernel);
}
