...
__device__ __forceinline__ uint16_t row_major_matrix_apply(const uint16_t* rows, const uint16_t sampleIx)
{
    // Text-exact port of Nabla RowMajorSobolMatrix::operator().
    // It computes four output rows at a time and uses a strided mask
    // 0b10000100001 to keep the parity bit at bit positions 0,5,10,15.
    const uint16_t keepMask = uint16_t(0b10000100001u);

    uint16_t val = uint16_t(__popc(uint32_t(rows[0] & sampleIx)));
    val |= uint16_t(__popc(uint32_t(rows[5]  & sampleIx)) << 5);
    val |= uint16_t(__popc(uint32_t(rows[10] & sampleIx)) << 10);
    val |= uint16_t(__popc(uint32_t(rows[15] & sampleIx)) << 15);
    val &= keepMask;

#pragma unroll
    for (uint16_t i = 1; i < 5; i++)
    {
        uint16_t tmp = uint16_t(__popc(uint32_t(rows[i]      & sampleIx)));
        tmp |= uint16_t(__popc(uint32_t(rows[5 + i]  & sampleIx)) << 5);
        tmp |= uint16_t(__popc(uint32_t(rows[10 + i] & sampleIx)) << 10);
        val |= uint16_t((tmp & keepMask) << i);
    }
...
__global__ void sobol_bench_kernel(
    ...
    const QuantizedSequence3* pSampleBuffer, // BDA-equivalent flat pointer, unused for METHOD 1
    PushConstants pc)
{
...
#pragma unroll
                for (uint16_t c = 0; c < COMPONENTS; c++)
                {
                    ...
                    triples[t][c] = row_major_matrix_apply(g_matrixSlots[slot], uint16_t(s ^ rank));
                }
            }
...
int main()
{
    ...
    printf("METHOD=1, BENCH_ITERS=%d, WORKGROUP_SIZE=%d, DEPTH=%d\n",
    ...
    printf("[Benchmark] METHOD 1 | ps/sample: %.3f | GSamples/s: %.3f | ms total: %.3f\n",
